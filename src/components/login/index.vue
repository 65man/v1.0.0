  ...
  <div id="login_lo">
      ...
      <div class="form">
        <group label-width="4em" label-margin-right="1em" label-align="right">
          ...
          <div class="tips-popover-border">
            <x-input title="账号" placeholder="请输入账号" v-model="phone" :max="11" is-type="china-mobile" :should-toast-error='false'
          ...
          <!-- 密码 -->
          <div class="tips-popover-border">
            <x-input title="密码" type="password" placeholder="请输入密码" v-model="password" :mix="6" :max="12" @keyup.13.native="sendlogin()"
               @on-focus='passwordFocus'  :should-toast-error='false'></x-input>
          ...
          <!-- 验证码 -->
          <!-- <x-input title="验证码" placeholder="请输入验证码" v-model="code" class="weui-cell_vcode">
            <img slot="right" class="weui-vcode-img" src="../../assets/img/bg.jpg">
          </x-input> -->

        </group>
        <div class="agm">
           <div>
                <input type="checkbox"  v-model="rememberMe" value="agm" id='radio1'>
            <label for="radio1">记住账号</label>
           </div>
           <div> <input type="checkbox"  v-model="selected" value="agm"  id='radio2'>
         <label for="radio2">我已阅读并同意</label>
          <a href="javascript:;" @click="show_agm">《合作协议》</a></div>
        </div>
        <x-button type="primary" action-type="button" @click.native="sendlogin" class="btn1">登录</x-button>
        <x-button  v-if="wechat=='true'" type="weChat" action-type="button" class="btn2" @click.native='weChatLogin'>使用微信登录</x-button>
      </div>
    </div>

    <!-- 底部 -->
    <!-- <div class="login_footer">
      <x-button type="default" action-type="button" class="btn3" @click.native="goRegister">注册账号</x-button>
      <x-button type="default" action-type="button" class="btn4" @click.native="findPassword">忘记密码</x-button>
    </div> -->
    ...
    <x-dialog v-model="showAgreement" :dialog-style="{'max-width': '980px', width: '100%', height: '100%', 'background-color': 'transparent'}">
      <div class="show_agm">
        ...
        <div class="amg_btn">
          <x-button type="primary" mini action-type="button" @click.native="hide_agm">我已阅读并同意以上条款</x-button>
        </div>
      </div>
    </x-dialog>
